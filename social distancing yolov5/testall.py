...
mixer.init()
sound = mixer.Sound('masksiri.wav')
sound2 = mixer.Sound('dis.wav')
chtime = False
w_real = 420
h_real = 360
tl = (254,294)
...
parser.add_argument('--source', type=str, default='progess.mp4', help='source')
opt = parser.parse_args()

set_logging()

device = select_device()
model = attempt_load('lastmask.pt', map_location=device)  # load FP32 model
model2 = attempt_load('yolov5s.pt', map_location=device)  # load FP32 model ++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
print('names',names2)
colors = [[random.randint(0, 255) for _ in range(3)] for _ in names2]
...
cap = cv2.VideoCapture('D:/Final_Project/dis/Social Distance/mask2.mp4') #############################################++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++video
...
while True:
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if(names[int(cls)] == 'with_mask'):
                	print('NO Beep')
                elif(names[int(cls)] == 'no_mask'):
                	if chtime == False:
                		chtime=True
                		delay = time.time()
                		sound.play()
                	print('Beep')
                	if time.time() - delay > 8 :
                		chtime = False
    img2 = letterbox(img0, stride=stride)[0]
    ...
    pred2 = non_max_suppression(pred2,0.7)
    t2 = time_synchronized()
    #print('')
    ...
    for i, det in enumerate(pred2):  # detections per image
        s = ''
        if len(det):
            det[:, :4] = scale_coords(img2.shape[2:], det[:, :4], img0.shape).round()
            for c in det[:, -1].unique():
                n = (det[:, -1] == c).sum()  # detections per class
